...
<head>
    ...
    <style>
        ...
        .camion-marker { animation: bounce 0.5s ease infinite; z-index: 1000; }
        @keyframes bounce { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-5px); } }
        .camion-activo { border: 3px solid #f59e0b; }
        .info-camion { background: #fef3c7; border-left: 4px solid #f59e0b; padding: 8px 12px; border-radius: 12px; margin: 12px; font-size: 0.75rem; display: flex; align-items: center; justify-content: space-between; }
        .camion-offline { filter: grayscale(0.5); opacity: 0.6; }
    </style>
</head>
<body>
    <div class="header-gradient p-4 text-white">
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                <p class="small opacity-75 mb-0">Oxkutzcab, Yucatán - Rutas del camión en tiempo real</p>
            </div>
        </div>
    </div>
    
    <!-- Información del camión en tiempo real -->
    <div id="infoCamion" class="info-camion" style="display: none;">
        <div>
            <i class="fas fa-truck text-warning"></i>
            <span id="camionEstado">Buscando camión...</span>
        </div>
        <div id="camionUbicacion" class="small"></div>
    </div>
    ...
    let intervaloCamion = null;
    let camionActivo = false;
    ...
    // ============================================
    function iniciarSeguimientoCamion() {
        if(intervaloCamion) clearInterval(intervaloCamion);
        
        intervaloCamion = setInterval(() => {
            const posicion = localStorage.getItem('simulador_ultima_posicion');
            
            if(posicion) {
                try {
                    const data = JSON.parse(posicion);
                    const tiempoTranscurrido = (new Date() - new Date(data.timestamp)) / 1000;
                    const esReciente = tiempoTranscurrido < 10;
                    ...
                    if(esReciente) {
                        ...
                        actualizarMarcadorCamion(data.lat, data.lng, data.nombre, data.punto, data.ruta);
                        $('#camionEstado').html(`<i class="fas fa-circle text-success"></i> Camión en ruta: ${data.ruta || 'Activo'}`);
                        $('#camionUbicacion').html(`📍 Último punto: ${data.nombre || 'En movimiento'} | Punto ${data.punto || '?'}`);
                    } else {
                        if(camionActivo) {
                            camionActivo = false;
                            $('#camionEstado').html(`<i class="fas fa-circle text-secondary"></i> Camión no disponible`);
                            $('#camionUbicacion').html(`⏰ Última actualización hace ${Math.round(tiempoTranscurrido)} segundos`);
                        }
                        
                        if(camionMarker) {
                            camionMarker.setOpacity(0.5);
                        }
                    }
                } catch(e) {
                    console.error('Error al procesar posición del camión:', e);
                }
            } else {
                if(camionActivo) {
                    camionActivo = false;
    ...
    function actualizarMarcadorCamion(lat, lng, nombre, punto, ruta) {
        if(camionMarker) {
            camionMarker.setLatLng([lat, lng]);
            camionMarker.setOpacity(1);
            camionMarker.setPopupContent(`
                <div style="text-align: center;">
                    <i class="fas fa-truck fa-2x text-warning"></i>
                    <strong>🚛 Camión en movimiento</strong><br>
                    Ruta: ${ruta || 'Activa'}<br>
                    Próximo punto: ${nombre || 'En recorrido'}<br>
                    <small>📍 Punto ${punto || '?'}</small>
                </div>
            `);
        } else {
            const camionIcon = L.divIcon({
                html: `<div style="background-color: #f59e0b; width: 34px; height: 34px; border-radius: 50%; border: 3px solid white; box-shadow: 0 0 12px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center;">
            ...
            camionMarker = L.marker([lat, lng], { icon: camionIcon })
                .addTo(map)
                .bindPopup(`
                    <div style="text-align: center;">
                        <i class="fas fa-truck fa-2x text-warning"></i>
                        <strong>🚛 Camión en movimiento</strong><br>
                        Ruta: ${ruta || 'Activa'}<br>
                        Próximo punto: ${nombre || 'En recorrido'}
                    </div>
                `);
        }
        
        // Centrar el mapa en el camión si el usuario no está interactuando
        if(!map._interacting && camionActivo) {
